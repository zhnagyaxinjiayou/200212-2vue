    ...
    <section class="con">
      ...
      <div class="conPoin" v-if="detailInfo.categoryView">
        <!-- 
          ...
          id:61
         -->
         <!-- detailInfo初始值是{}.categoryView 会产生 undefinde -->
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in detailInfo.spuSaleAttrList" :key="attr.id">
                <dt class="title">{{attr.saleAttrName}}</dt>
                <dd :class="{active:value.isChecked==='1'}" v-for="(value, index) in attr.spuSaleAttrValueList" 
...
import {mapState,mapGetters} from 'vuex'
  ...
  export default {
    ...
    async  addToCart(){
      ...
      // const errorMsg=await this.$router.dispatch('addToCart3',{skuId,skuNum})
      // if(errorMsg){
      //   // 失败
      //   alert(errorMsg)
      // }else{
      //   // 成功:  保存query参数，跳转依然携带参数，使用sessionStorage(浏览器刷新依然存在，当浏览器关闭时不存在)
    ...
    computed: {
      ...mapState({
        // 映射detailInfo名字的数据，detail模块.detailInfo这个属性
        detailInfo:state=>state.detail.detailInfo
      }),
      ...mapGetters(['getDetailInfo','skuInfo','skuImageList'])
    },
    
    // 将数据从接口请求获取到状态里
    mounted() {
      // vuex中 actions的名字 "getDetailInfo"，id
