...
import Vue from 'vue'
  ...
  export default {
    ...
    methods: {
      // 设置新的排序
      setOrder(flag){//"1"综合，"2"价格
      // 取出原本的ordeFlag和 orderType
      let [ordeFlag,orderType]=this.options.order.split(':')
      // 点击当前排序项：切换排序方式（顺序不变）
      if(flag===ordeFlag){
        orderType=orderType==='acs' ? 'desc':'asc'
      }else{
      ...
      // 更新order
      this.options.order=ordeFlag+':'+orderType
      // 重新请求获取列表数据显示
      this.getProductList()

      },
      isActive(ordeFlag){//'1'/'2'
        return this.options.order.indexOf(ordeFlag)===0
      },
      // 删除属性条件
      removeProp(index){
        // 删除对应的属性条件
      ...
      updateOptions(){
        ...
        // 更新options
        this.options={
          // 1.先取出对象里面所有的属性放在这个里面
          ...this.options,
          // 2.再将参数的数据放进来，对以前的进行覆盖的效果 this.options.categoryName=''
          categoryName,
          category1Id,
          category2Id,
          category3Id,
      ...
      removeKeyword(){
        this.options.keyword = ''
           // 重新请求列表数据
      this.getProductList()
      // 重新跳转到当前Search, 干掉分类的params参数
        // params参数不要了，只要query参数
